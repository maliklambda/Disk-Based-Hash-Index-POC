...
The variable-length of an entry makes it more complex to store and search for it, compared to entries of fixed size. 
While slotted pages make it easier, it is far from a perfect solution.

Hash-indexing is better in that regard. 
However, most hash-indices require to the entire hash-array to be in memory.
...
It is basically a middle-ground between a traditional Btree- and a Hash-index.
Use cases for such a Disk-Based-Hash-Index (DBHI) would be situations, in which a Hash-index would have been chosen, 
but memory constraints do not allow the entire index to be read into RAM at once.

## How it works
The base of a DBHI is a regular Btree.
...
![DBHI Architecture diagram](./static/disk_based_hash_index_poc.png "DBHI Architecture")
...
*Note that for the POC in this repo, the Btree still sits in memory.<br>
...
A DBHI comes with several trade-offs when compared to established index methodology.
Most comparisons in the following are made to on-disk-Btrees, since they are most similar to DBHIs.
### Positives
- Reduced disk space: 
Values are not stored redundantly, as they are in regular Btrees. 
Each value is stored only in the entry it references and not in the tree itself.
- Reduced complexity:
The complexity that comes with storing variable-length data is eliminated fully.
- Faster Tree restructuring: 
Restructuring the Btree is faster due to the reduced complexity that comes with storing fixed-size values.
- Faster insertion, update and delete operations:
Due to the faster restructuring of the Btree, all destructive operations are faster.
- Faster Index-construction and recovery:
Faster restructuring means faster construction and faster recovery.


### Negatives
- Range queries are not supported (other than full-table scans). 
Neither is pattern matching (e.g. "LIKE 'front%s'").
...
which would yield the content-file to no longer be append-only.
...
as well as multiple entry-reads. 
This event is very unlikely, must however be considered, especially for large input sizes.
